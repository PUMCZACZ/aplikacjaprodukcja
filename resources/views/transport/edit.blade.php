            ...
            <main class="flex-1">
                <div class="border border-gray-300 p-6 rounded-xl">
                    <form method="POST" action="{{ route('transports.update', $transport->id) }}">
                        ...
                        <x-form.input name="product_amount" :value="old('product_amount', $transport->product_amount)">Ilość</x-form.input>
                        <x-form.input name="product_price" type="number" step="1" min="1" required :value="old('product_price', $transport->product_price)">Cena</x-form.input>
                        <x-form.input name="delivered_at" type="datetime-local" value="{{ now() }}" required :value="old('delivered_at', $transport->delivered_at)">Data Dostarczenia</x-form.input>
